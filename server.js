...
app.post("/api/conclusoes", async (req, res) => {
  try {
    ...
    // Cria tabela conclusoes se não existir (opcional para segurança)
    await pool.query(`
    ...
    // Insere a conclusão no banco
    const [result] = await pool.query(
      "INSERT INTO conclusoes (ideia_id, video, imagens, descricao) VALUES (?, ?, ?, ?)",
...
// 📥 Listar conclusões de uma ideia específica
app.get("/api/conclusoes/:ideia_id", async (req, res) => {
  try {
    ...
    console.error("❌ Erro ao buscar conclusões da ide
